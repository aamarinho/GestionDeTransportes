...
{
    public class GraficoCliente : Form
    {
		...
		public GraficoCliente(string nombre)
        {
			try
			{
				...
				foreach (XElement level1Element in XElement.Load("Transportes.xml").Elements("transporte"))
				{
					...
					String toret = level1Element.Element("cliente").Value;
					//string nombre;
				...
				//Console.WriteLine("Error al insertar nombre,poner uno que exista", e);
				new PanelAviso();
